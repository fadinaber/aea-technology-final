...
function DistributorCard({
  ...
  // Extract clean URL for display (remove protocol and trailing slash)
  ...
  return (
    <Card className="group h-full overflow-hidden border border-border hover:border-primary/40 hover:shadow-lg transition-all duration-300 bg-card">
      <div className="p-6 flex flex-col h-full gap-4">
        ...
        {/* Contact Information Section with consistent spacing */}
...
function USDistributorCard({ distributor }: { distributor: USDistributor }) {
  return (
    <DistributorCard
...
function InternationalDistributorCard({ item }: { item: FlattenedDistributor }) {
  const { country, distributor } = item
  return (
    <DistributorCard
      category={country}
      name={distributor.name}
...
export function DistributorLocator() {
  ...
  const filteredDistributors = useMemo(() => {
    ...
    if (!query) return flattenedDistributors

    return flattenedDistributors.filter((item) => item.countryLower.includes(query))
  }, [searchQuery])
  ...
  const showBuyDirectFallback = useMemo(() => {
    ...
    return false
  }, [searchQuery, filteredDistributors.length])
          ...
          <section aria-label="US Distributors">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 max-w-5xl mx-auto">
              {usDistributors.map((distributor) => (
                <USDistributorCard key={distributor.id} distributor={distributor} />
              ))}
            </div>
          </section>
        )}
            ...
            {showBuyDirectFallback ? (
              ...
              <>
                <p className="text-sm text-slate-500 mb-6 text-center" aria-live="polite">
                  {searchQuery
                    ? `Showing ${filteredDistributors.length} result${filteredDistributors.length !== 1 ? "s" : ""}`
                    : `${flattenedDistributors.length} distributors worldwide`}
                </p>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredDistributors.map((item) => (
                    <InternationalDistributorCard key={`${item.country}-${item.distributor.id}`} item={item} />
                  ))}
                </div>
